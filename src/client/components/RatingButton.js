...
class RatingButton extends React.PureComponent {
    handleClick = event => {
        ...
        const { currentTarget: { attributes: { rating: { value } } } } = event;
        onChange(value);
    };
    ...
    render() {
        ...
        const buttons = Object.values(RatingType)
            ...
            .map(rating => {
                ...
                return (
                    <IconButton key={rating} rating={rating} className={`${rating} ${activeClass}`} aria-label="Show form" onClick={this.handleClick}>
                        <Icon />
                    </IconButton>
                );
            });
